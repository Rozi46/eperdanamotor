...
use App\Models\{Setting, Kantor, User, LevelAdmin, ListAkses, Activity, Customer, Penjualan};
...
class ApiServiceCustomer
{
    public function listcustomer($request)
    {
        ...
        if(!$viewadmin){
            ...
            $level_sub_menu = LevelAdmin::where('code_data', $viewadmin->level)->where('data_menu','=','listcustomer')->first();
            $level_action = LevelAdmin::where('code_data', $viewadmin->level)->where('data_menu','=','exportcustomer')->first();
            ...
            $results['listdata'] = Customer::where(function($query) use ($request) {
            ...
            foreach($results['listdata'] as $key => $data){
                $results['count_used'][$data->id] = Penjualan::where('kode_customer', $data->id)->count();
            }
    ...
    public function newcustomer($request)
    {
        ...
        if($viewadmin){
            $level_menu = LevelAdmin::where('code_data', $viewadmin->level)->where('data_menu','=','listcustomer')->first();
            $level_action = LevelAdmin::where('code_data', $viewadmin->level)->where('data_menu','=','newcustomer')->first();
            ...
            $validator = Validator::make($request->all(), [
                'nama' => 'required|string|max:200|unique:db_customer',
            ...
            if($savedata){
                ...
                Activity::create([
                    ...
                    'activity' => 'Tambah data customer ['.$request->nama.' - '.$newCodeData.']',
    ...
    public function viewcustomer($request)
    {
        ...
        if($viewadmin){
            $level_menu = LevelAdmin::where('code_data', $viewadmin->level)->where('data_menu','=','masterdata')->first();
            $level_action = LevelAdmin::where('code_data', $viewadmin->level)->where('data_menu','=','listcustomer')->first();
            
            if($level_menu->access_rights == 'No' OR $level_action->access_rights == 'No'){
                return response()->json(['status_message' => 'error','note' => 'Tidak ada akses','results' => $object]);
            }
            $getdata['customer'] = Customer::where('id', $request->id)->first();
            if($getdata['customer']){ 
                $count_used = Penjualan::where('kode_customer', $getdata['customer']->id)->count();            
    ...
    public function editcustomer($request)
    {
        ...
        if($viewadmin){
            $level_menu = LevelAdmin::where('code_data', $viewadmin->level)->where('data_menu','=','listcustomer')->first();
            $level_action = LevelAdmin::where('code_data', $viewadmin->level)->where('data_menu','=','editcustomer')->first();
            
            if($level_menu->access_rights == 'No' OR $level_action->access_rights == 'No'){
                return response()->json(['status_message' => 'error','note' => 'Tidak ada akses','results' => $object]);
            }
            $getdata = Customer::where('id', $request->id)->first();
            if($getdata){
                $validator = Validator::make($request->all(), [
                ...
                if($request->nama != $getdata->nama){
                    $validator = Validator::make($request->all(),['nama'=>'required|string|max:200|unique:db_customer']);
                    if($validator->fails()){
                        return response()->json(['status_message' => 'error','note' => $validator->errors(),'results' => $object]);
                    }
                }
                
                $updatedata = Customer::where('id', $request->get('id'))
                    ->update([
                        'nama' => ucfirst($request->get('nama')),
                        'no_telp' => $request->get('no_hp'),
                        'alamat' => ucfirst($request->get('alamat')),
                ...
                if($updatedata){
                    ...
                    Activity::create([
                        ...
                        'code_data' => $newCodeData_activity,
                        'kode_user' => $viewadmin->id,
                        'activity' => 'Update data customer ['.$getdata->nama.' - '.$getdata->code_data.']',
                        'kode_kantor' => $viewadmin->kode_kantor,
                    ]);
                    return response()->json(['status_message' => 'success','note' => 'Data berhasil disimpan','id_data' => Str::uuid(),'results' => $object]);
                }else{
    ...
    public function upstatuscustomer($request)
    {
        $object = [];
        $viewadmin = User::where('id', $request->u)->where('key_token', $request->token)->first();
            ...
            if($getdata){
                ...
                if($updatedata){
                    ...
                    Activity::create([
                        ...
                        'code_data' => $newCodeData_activity,
                        'kode_user' => $viewadmin->id,
                        'activity' => 'Update status customer ['.$getdata->nama.' - '.$getdata->code_data.']',
                        'kode_kantor' => $viewadmin->kode_kantor,
                    ]);
    ...
    public function deletecustomer($request)
    {
        ...
        if(!$viewadmin){
            ...
            $level_menu = LevelAdmin::where('code_data', $viewadmin->level)->where('data_menu','=','listcustomer')->first();
            $level_action = LevelAdmin::where('code_data', $viewadmin->level)->where('data_menu','=','deletecustomer')->first();
            if($level_menu->access_rights == 'No' OR $level_action->access_rights == 'No'){
                return response()->json(['status_message' => 'error','note' => 'Tidak ada akses','results' => $object]);
            }
                $getdata = Customer::where('id', $request->id)->first();
                if($getdata){
                    ...
                    if($DelData){
                        ...
                        Activity::create([
                            ...
                            'code_data' => $newCodeData_activity,
                            'kode_user' => $viewadmin->id,
                            'activity' => 'Hapus data customer ['.$getdata->nama.' - '.$getdata->code_data.']',
                            'kode_kantor' => $viewadmin->kode_kantor,
                        ]);
