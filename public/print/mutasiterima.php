<?php
    ...
    $request = new Request([
        'u' => $admin_login,
        'token' => $key_token,
        'nomor_mutasi_terima' => $print_code
    ]);    
    ...
    $response = app(\App\Services\ApiServiceMutasiterima::class)->viewmutasiterima($request);
    ...
    class PDF extends MultiCellTable
    {
        ...
        function Header()
        {
            ...
            $this->rowHeaderDetail(
                'No. Mutasi Terima',$getdata['detail_mutasi_terima']['nomor'] ?? 'Belum ditentukan',
                'No. Mutasi Kirim',$getdata['detail_mutasi']['nomor'] ?? 'Belum ditentukan'
            );

            $this->SetFont('Arial','',9);
            $this->rowHeaderDetail(
                'Tanggal Terima',isset($getdata['detail_mutasi_terima']['tanggal']) ? date('j F Y', strtotime($getdata['detail_mutasi_terima']['tanggal'])) : 'Belum ditentukan',
                'Tanggal Kirim',isset($getdata['detail_mutasi']['tanggal']) ? date('j F Y', strtotime($getdata['detail_mutasi']['tanggal'])) : 'Belum ditentukan'
            );
        ...
        function Footer()
        {
            ...
            $this->Line(141,$topTable,141,$yFooter-2);
            ...
            $this->SetFont('Arial','',9);
            $this->Cell(12);
            $this->MultiCell(148,4,$data['detail_mutasi_terima']['ket'] ?? 'Belum ditentukan');
        ...
        function AddContent()
        {
            ...
            foreach($data['list_produk'] as $item){
                ...
                $nama=$data['detail_produk'][$id]['nama'] ?? 'Belum ditentukan';
                $qty_mutasi=number_format($item['jumlah_kirim'] ?? 0,2,",",".").' '.($data['satuan_produk'][$id]['nama'] ?? '');  $qty_terima=number_format($item['jumlah_terima'] ?? 0,2,",",".");

                $row=[$no,$nama,$qty_mutasi,$qty_terima];
