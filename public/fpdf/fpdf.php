...
function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='')
{
	...
	if($this->y + (float)$h > $this->PageBreakTrigger && !$this->InHeader && !$this->InFooter && $this->AcceptPageBreak())
	{
		// Automatic page break
		$x = $this->x;
		$ws = $this->ws;
	...
	if($txt!=='')
	{
		...
		$x = ((float)$this->x + (float)$dx) * (float)$k;
		$y = ((float)$this->h - ((float)$this->y + 0.5*(float)$h + 0.3*(float)$this->FontSize)) * (float)$k;
