					...
					<div class="row">
							...
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
                                        ...
                                        @endif
                                        @if($level_user['inputpengeluarankas'] == 'Yes')
						...
						<div class="col-md-12 bg_page_main form_action" line="form_action">
							<div class="col-md-12 data_page">
								<form method="post" name="form_data" enctype="multipart/form-data" action="/admin/saveppengeluarankas">
									{{csrf_field()}}
                                            ...
                                            <div class="form-group row form_input text-left">
                                                ...
                                                <div class="col-sm-8 input">
                                                    <select name="akun_biaya" placeholder="Akun biaya">	
                                                        <option value="" selected="true">Pilih Akun</option>
                                                        @foreach ($list_akun as $view_data)
                                                            <option value="{{$view_data['id']}}">{{$view_data['kode']}} - {{$view_data['nama']}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
			...
			@section('script')
				<script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        function getcodepaykas(){     
                            var tgl_transaksi = $('input[name="in_tgl_transaksi"]').val(); 
                            $.getJSON("/admin/getcodepaykas?token=<?php echo $request['token'];?>&u=<?php echo $request['u'];?>&tgl_transaksi="+tgl_transaksi, function(results){
                        ...
                        $('button[name="btn_save"]').click(function(){
                            ...
                            if($(('button[name="btn_save"]')).click){
                                ...
                                $('div[data-model="confirmasi"] .modal-body').html('<div class="alert alert-warning">Anda yakin untuk simpan pengeluaran kas ' + nomor_transaksi + '. <br>Setelah simpan maka data tidak bisa diubah kembali.</div>');
