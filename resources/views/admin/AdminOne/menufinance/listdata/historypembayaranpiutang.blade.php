...
@section('title', 'History Pembayaran Penjualan')
                ...
                <div class="container-fluid text-left">
                    <div class="row">
                        <div class="col-md-12 bg_page_main hd">
                            <div class="col-md-12 hd_page_main">History Pembayaran Penjualan</div>
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['inputpembayaranpiutang'] == 'Yes')<a href="/admin/menupembayaranpiutang"><button type="button" class="btn btn-primary">Input Pembayaran</button></a>@endif
										
										@if($level_user['exportpembayaranpiutang'] == 'Yes')<button type="button" class="btn btn-info back" onclick="exportdata('salespayment')"><i class="fa fa-download"></i> Export Data</button>@endif
									</div>
								</div>
							</div>
                        </div>
										...
										<table class="table_view table-striped table-hover">
											<thead>
												<tr>
													...
													<th class="colleft" style="width:200px; text-align: center;">No. Pembayaran</th>
                                                    <th style="width:250px; text-align: center;">Pembayaran Penjualan Oleh</th>
                                                    <th style="min-width:175px; text-align: center;">Nomor Penjualan</th>
												...
												<?php $no = 0;?> @forelse($results['data'] as $view_data) <?php $no++ ;?>
													<script>
														function viewdata_{{$no}}() {
															loadingpage(2000);
															window.location.href = "/admin/viewsalespayment?d={{$view_data['code_data']}}";
														}
                                                        function printdata_{{$no}}() {
                                                            window.open("/admin/printsalespayment?d={{$view_data['code_data']}}");
                                                        }
													</script>
													<tr>
                                                        ...
                                                        <td>{{ $view_data['nomor_piutang'] ?? 'Belum ditentukan' }} - [ {{ $listdata['detail_customer'][$view_data['code_data']]['nama'] ?? 'Belum ditentukan' }} ]</td>
														<td style="text-align:right;">{{ number_format($view_data['jumlah'] ?? 0,0,"",".") }}</td>
			...
			@section('script')
				<script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        $('[line="btn_page_awal"]').attr('href','/admin/historypembayaranpiutang?d={{$request['d']}}&page=1&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_min"]').attr('href','/admin/historypembayaranpiutang?d={{$request['d']}}&page={{ $results['current_page'] - 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_plus"]').attr('href','/admin/historypembayaranpiutang?d={{$request['d']}}&page={{ $results['current_page'] + 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

						$('[line="btn_page_akhir"]').attr('href','/admin/historypembayaranpiutang?d={{$request['d']}}&page={{ $results['last_page'] }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');
                    });
					...
					function datefilter() {
						...
						if($('input[name="datefilterstart"]').val() != '' && $('input[name="datefilterend"]').val() != ''){
							window.location.href = "/admin/historypembayaranpiutang?d={{$request['d']}}&page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+key_search+"&searchdate="+datefilterstart+"sd"+datefilterend;
						}else{
					...
					function exportdata() {
						...
						window.location.href = "/admin/exportpembayaranpiutang?page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+encodeURIComponent($('input[name="key-search"]').val())+searchdate;
					}
                </script>
            @endsection
@endsection