...
@section('title', 'History Penerimaan Kas')
                ...
                <div class="container-fluid text-left">
                    <div class="row">
                        <div class="col-md-12 bg_page_main hd">
                            <div class="col-md-12 hd_page_main">History Penerimaan Kas</div>
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['inputpenerimaankas'] == 'Yes')<a href="/admin/menupenerimaankas"><button type="button" class="btn btn-primary">Input Penerimaan Kas</button></a>@endif
										
										@if($level_user['exportpenerimaankas'] == 'Yes')<button type="button" class="btn btn-info back" onclick="exportdata('penerimaankas')"><i class="fa fa-download"></i> Export Data</button>@endif
									</div>
								</div>
							</div>
                        </div>
										...
										<table class="table_view table-striped table-hover">
											<thead>
												<tr>
													...
													<th class="colleft" style="width:200px; text-align: center;">No. Penerimaan Kas</th>
                                                    <th style="min-width:175px; text-align: center;">Penerimaan Kas Oleh</th>
												...
												<?php $no = 0;?> @forelse($results['data'] as $view_data) <?php $no++ ;?>
													<script>
														function viewdata_{{$no}}() {
															loadingpage(2000);
															window.location.href = "/admin/viewpenerimaankas?d={{$view_data['code_data']}}";
														}
                                                        function printdata_{{$no}}() {
                                                            window.open("/admin/printpenerimaankas?d={{$view_data['code_data']}}");
                                                        }
													</script>
													<tr>
                                                        ...
                                                        <td>{{ $view_data['jenis'] ?? 'Belum ditentukan' }}</td>
														<td style="text-align:right;">{{ number_format($view_data['nilai'] ?? 0,0,"",".") }}</td>
			...
			@section('script')
				<script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        $('[line="btn_page_awal"]').attr('href','/admin/historypenerimaankas?d={{$request['d']}}&page=1&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_min"]').attr('href','/admin/historypenerimaankas?d={{$request['d']}}&page={{ $results['current_page'] - 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_plus"]').attr('href','/admin/historypenerimaankas?d={{$request['d']}}&page={{ $results['current_page'] + 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

						$('[line="btn_page_akhir"]').attr('href','/admin/historypenerimaankas?d={{$request['d']}}&page={{ $results['last_page'] }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');
                    });
					...
					function datefilter() {
						...
						if($('input[name="datefilterstart"]').val() != '' && $('input[name="datefilterend"]').val() != ''){
							window.location.href = "/admin/historypenerimaankas?d={{$request['d']}}&page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+key_search+"&searchdate="+datefilterstart+"sd"+datefilterend;
						}else{
					...
					function exportdata() {
						...
						window.location.href = "/admin/exportpenerimaankas?page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+encodeURIComponent($('input[name="key-search"]').val())+searchdate;
					}
                </script>
            @endsection
@endsection