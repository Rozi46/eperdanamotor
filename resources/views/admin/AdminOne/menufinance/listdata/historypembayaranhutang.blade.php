...
@section('title', 'History Pembayaran Pembelian')
                ...
                <div class="container-fluid text-left">
                    <div class="row">
                        <div class="col-md-12 bg_page_main hd">
                            <div class="col-md-12 hd_page_main">History Pembayaran Pembelian</div>
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['inputpembayaranhutang'] == 'Yes')<a href="/admin/menupembayaranhutang"><button type="button" class="btn btn-primary">Input Pembayaran</button></a>@endif
										
										@if($level_user['exportpembayaranhutang'] == 'Yes')<button type="button" class="btn btn-info back" onclick="exportdata('purchasepayment')"><i class="fa fa-download"></i> Export Data</button>@endif
									</div>
								</div>
							</div>
                        </div>
										...
										<table class="table_view table-striped table-hover">
											<thead>
												<tr>
                                                    ...
                                                    <th style="width:250px; text-align: center;">Pembayaran Pembelian Oleh</th>
                                                    <th style="min-width:175px; text-align: center;">Nomor Pembelian</th>
												...
												<?php $no = 0;?> @forelse($results['data'] as $view_data) <?php $no++ ;?>
													<script>
														function viewdata_{{$no}}() {
															loadingpage(2000);
															window.location.href = "/admin/viewpurchasepayment?d={{$view_data['nomor']}}";
														}
                                                        function printdata_{{$no}}() {
                                                            window.open("/admin/printpurchasepayment?d={{$view_data['nomor']}}");
                                                        }
													</script>
													<tr>
                                                        ...
                                                        <td>{{ $view_data['nomor_hutang'] ?? 'Belum ditentukan' }} - [ {{ $listdata['detail_supplier'][$view_data['code_data']]['nama'] ?? 'Belum ditentukan' }} ]</td>
			...
			@section('script')
				<script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        $('[line="btn_page_awal"]').attr('href','/admin/historypembayaranhutang?d={{$request['d']}}&page=1&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_min"]').attr('href','/admin/historypembayaranhutang?d={{$request['d']}}&page={{ $results['current_page'] - 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_plus"]').attr('href','/admin/historypembayaranhutang?d={{$request['d']}}&page={{ $results['current_page'] + 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

						$('[line="btn_page_akhir"]').attr('href','/admin/historypembayaranhutang?d={{$request['d']}}&page={{ $results['last_page'] }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');
                    });
					...
					function datefilter() {
						...
						if($('input[name="datefilterstart"]').val() != '' && $('input[name="datefilterend"]').val() != ''){
							window.location.href = "/admin/historypembayaranhutang?d={{$request['d']}}&page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+key_search+"&searchdate="+datefilterstart+"sd"+datefilterend;
						}else{
					...
					function exportdata() {
						...
						window.location.href = "/admin/exportpembayaranhutang?page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+encodeURIComponent($('input[name="key-search"]').val())+searchdate;
					}
                </script>
            @endsection
@endsection