				...
				<div class="container-fluid text-left">
					<div class="row">
						<div class="col-md-12 bg_page_main hd" line="hd_action">
							<div class="col-md-12 hd_page_main">Pengeluaran Kas</div>
								...
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
                                        ...
                                        @if($level_user['historypengeluarankas'] == 'Yes')
                                            <a href="/admin/historypengeluarankas"><button type="button" class="btn btn-success" btn="history_data">History Pengeluaran Kas</button></a>
                                            
                                            <a href="printpengeluarankas?d={{$results['results']['detail']['code_data']}}" target="_blank"><button type="button" class="btn btn-secondary" name="btn_print" btn="print_data"><i class="fa fa-print"></i> Print Pengeluaran Kas</button></a>
                                            
                                            <?php if($res_user['level'] == 'LV5677001'){?> <button type="button" class="btn btn-danger" name="del_data" btn="del_data"><i class="fa fa-trash-o"></i> Hapus Pengeluaran Kas</button> <?php } ?> 
                                        @endif  
									</div>
								</div>
							</div>
						...
						<div class="col-md-12 bg_page_main form_action" line="form_action">
							<div class="col-md-12 data_page">
								<form method="post" name="form_data" enctype="multipart/form-data" action="/admin/savepengeluaran">
									{{csrf_field()}}
									<div class="row bg_data_page form_page content">
										...
										<div class="col-md-12 bg_form_page">
                                            <div class="form-group row form_input text-left">
                                                <label for="full_name" class="col-sm-2 col-form-label">Dibayarkan Oleh</label>
										...
										<div class="col-md-6 bg_form_page">
                                            <div class="form-group row form_input text-left">
                                                <label for="nomor_transaksi" class="col-sm-4 col-form-label">No. Pengeluaran Kas <span>*</span></label>
                                                <div class="col-sm-8 input">
			...
			@section('script')
				<script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        $('input[name="keterangan"]').val('{{ $results['results']['detail']['keterangan'] ?? 'Belum ditentukan'}}');
                    });

                    $('[name="del_data"]').click(function(){
                        if($('[name="del_data"]').click){
                            $('div[data-model="confirmasi"]').modal({backdrop: false});
                            $('div[data-model="confirmasi"] .modal-body').html('<div class="alert alert-danger">Anda yakin untuk menghapus pengeluaran kas {{$results['results']['detail']['nomor']}}.</div>');
                            ...
                            $('button[btn-action="aciton-confirmasi"]').click(function(){
                                if($('button[btn-action="aciton-confirmasi"]').click){
                                    ...
                                    loadingpage(20000);
                                    window.location.href = "/admin/deletepengeluarankas?d={{$results['results']['detail']['code_data']}}&tipe_data=pengeluarankas";
                                }  
                            });
                        }
                    });
