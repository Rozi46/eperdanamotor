					...
					<div class="row">
							...
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
                                        ...
                                        @if($level_user['editsupplier'] == 'Yes')
                                            <button type="button" class="btn btn-primary" name="btn_save" onclick="loadingpage(20000),SaveData('form_data')">Simpan Data</button>

                                            @if($results['count_used'] == 0)  @if($level_user['deletesupplier'] == 'Yes')<button type="button" class="btn btn-danger" name="btn_del" onclick="DeleteData()">Hapus Data</button>@endif @endif
                                        @endif
									</div>
								</div>
							</div>
                        ...
                        <div class="col-md-12 bg_page_main" line="form_action">
                            <div class="col-md-12 data_page">
                                <form method="post" name="form_data" enctype="multipart/form-data" action="/admin/editsupplier">
                                    {{csrf_field()}}
                                    <div class="row bg_data_page form_page content">
										<input type="text" name="id_data" value="{{ $results['results']['supplier']['id'] }}" readonly="true" style="display: none;" />
										<div class="col-md-6 bg_form_page">
											<div class="form-group row form_input text-left">
												<label for="nama" class="col-sm-4 col-form-label">Nama Supplier <span>*</span></label>
												<div class="col-sm-8 input">
													<input type="text" name="nama" placeholder="Nama Supplier" value="{{$results['results']['supplier']['nama']}}" autofocus/>
												</div>
											</div>
										</div>                          
                                        <div class="col-md-6 bg_form_page">
                                            <div class="form-group row form_input text-left">
                                                ...
                                                <div class="col-sm-8 input">
                                                    <input type="text" name="no_hp" placeholder="No Handphone" value="{{$results['results']['supplier']['no_telp']}}" onKeyPress="return goodchars(event,'0123456789,',this)"/>
                                                </div>
                                            </div>
                                        </div> 
										<div class="col-md-12 bg_form_page">
											<div class="form-group row form_input text-left">
												...
												<div class="col-sm-10 input">
													<input type="text" name="alamat" placeholder="Alanat" value="{{$results['results']['supplier']['alamat']}}" autofocus/>
												</div>
											</div>
										</div>
										<div class="col-md-12 bg_form_page">
											<div class="form_input text-left">
												<div class="bg_checkboxlios">
													<div class="tag_title">Status Supplier </div>
													<div class="checkboxlios" title="{{$results['results']['supplier']['status_data']}}">
			...
			@section('script')
                <script type="text/javascript">
                    $(document).ready(function(){
						...
						$('input[name="status_data"]').val('{{$results['results']['supplier']['status_data']}}');
                    ...
                    function DeleteData() {
                        ...
                        $('div[data-model="confirmasi"] .modal-body').html('<div class="alert alert-danger">Anda yakin untuk menghapus data {{ $results['results']['supplier']['nama'] }}.</div>');
                        ...
                        $('button[btn-action="aciton-confirmasi"]').click(function(){
                            if($('button[btn-action="aciton-confirmasi"]').click){
                                loadingpage(20000);
                                window.location.href = "/admin/deletesupplier?d={{ $results['results']['supplier']['id'] }}";
                            }
                        });
                    }
                </script>
