                ...
                <div class="container-fluid text-left">
                    <div class="row">
                        <div class="col-md-12 bg_page_main hd">
                            <div class="col-md-12 hd_page_main">Data Supplier</div>
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['newsupplier'] == 'Yes')<a load="true" href="/admin/newsupplier"><button type="button" class="btn btn-primary">Tambah Data</button></a>@endif
										
										@if($level_user['exportsupplier'] == 'Yes')<button type="button" class="btn btn-info back" onclick="exportdata('listsupplier')"><i class="fa fa-download"></i> Export Data</button>@endif
									</div>
								</div>
							</div>
                        </div>
										...
										<table class="table_view table-striped table-hover">
											<thead>
												<tr>
													...
													<th style="min-width:50px; text-align: center;">Kode Supplier</th>
													...
													<script type="text/javascript">
														$(document).ready(function(){
																...
																if($('[btn="del_data_{{$view_data['id']}}"]').click){
																	...
																	$('button[btn-action="aciton-confirmasi"]').click(function(){
																		if($('button[btn-action="aciton-confirmasi"]').click){
																			loadingpage(20000);
																			window.location.href = "/admin/deletesupplier?d={{$view_data['id']}}";
																		}
																	});
																}
															});
															...
															$('input[btn="btncheckbox_{{$view_data['id']}}"]').on('click', function(){
																loadingpage(20000);
																<?php if($level_user['editsupplier'] == 'Yes'){ ?>
																	$.ajax({
																		type: "GET",
																		url: "/admin/upstatussupplier?_token={{csrf_token()}}&token=<?php echo $request['token'];?>&u=<?php echo $request['u'];?>&id=<?php echo $view_data['id'];?>",
																		cache: false,
													...
													<tr>
														...
														<td style="text-align:center;">
															<div class="checkboxlios">
																...
																<input type="checkbox" class="ios" name="btncheckbox_{{$view_data['id']}}" btn="btncheckbox_{{$view_data['id']}}" style="display:none;" <?php if($level_user['editsupplier'] == 'No'){ ?>disabled="true"<?php }?>/>
															</div>
														</td>
														<td class="colright" style="text-align:center;">
															<div class="dropdown dropleft">
																...
																<div class="dropdown-menu">
																	...
																	<a load="true" class="dropdown-item" href="editsupplier?d={{$view_data['id']}}">Lihat/Ubah Data</a>
																	<a class="dropdown-item @if($listdata['count_used'][$view_data['id']] > 0) disabled @endif @if($level_user['deletesupplier'] == 'No') disabled @endif" <?php if($listdata['count_used'][$view_data['id']] == 0){ if($level_user['deletesupplier'] == 'Yes'){ ?> btn="del_data_{{$view_data['id']}}"<?php } }?>>Hapus Data</a>
																</div>
															</div>
														</td>
													</tr>
