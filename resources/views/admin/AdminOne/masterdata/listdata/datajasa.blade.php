                ...
                <div class="container-fluid text-left">
                    <div class="row">
                        <div class="col-md-12 bg_page_main hd">
                            <div class="col-md-12 hd_page_main">Data Jasa</div>
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['newjasa'] == 'Yes')<a load="true" href="/admin/newjasa"><button type="button" class="btn btn-primary">Tambah Data</button></a>@endif
										
										@if($level_user['exportjasa'] == 'Yes')<button type="button" class="btn btn-info back" onclick="exportdata('listjasa')"><i class="fa fa-download"></i> Export Data</button>@endif
									</div>
								</div>
							</div>
                        </div>
										...
										<table class="table_view table-striped table-hover">
											<thead>
												<tr>
													...
													<th style="min-width:250px; text-align: center;">Nama Jasa</th>
													<th style="min-width:50px; text-align: center;">Satuan</th>
													<th style="min-width:150px; text-align: center;">Modal Jasa</th>
													<th style="min-width:150px; text-align: center;">Biaya Jasa</th>
											...
											<tbody>
												<?php $no = 0;?> @forelse($results['data'] as $view_data) 
                                                        ...
                                                        $(document).ready(function(){
                                                            $('[btn="del_data_{{$view_data['id']}}"]').click(function(){
                                                                if($('[btn="del_data_{{$view_data['id']}}"]').click){
                                                                    ...
                                                                    $('div[data-model="confirmasi"] .modal-body').html('<div class="alert alert-danger">Anda yakin untuk menghapus jasa {{$view_data['nama']}}.</div>');
                                                                    ...
                                                                    $('button[btn-action="aciton-confirmasi"]').click(function(){
                                                                        if($('button[btn-action="aciton-confirmasi"]').click){
                                                                            loadingpage(20000);
                                											window.location.href = "/admin/deletejasa?d={{$view_data['id']}}";
                                                                        }
                                                                    });
                                                                }
                                                            });
													...
													<tr>
														...
														<td style="text-align:center;">{{$listdata['satuan'][$view_data['id']]['nama']}}</td>
														<td style="text-align: right;">{{$view_data['harga_beli']}} </td>
														<td style="text-align: right;">{{$view_data['harga_jual1']}} </td>
														<td class="colright" style="text-align:center;">
															<div class="dropdown dropleft">
																...
																<div class="dropdown-menu">
																	...
																	<a load="true" class="dropdown-item" href="editjasa?d={{$view_data['id']}}">Lihat/Ubah Data</a>
																	<a class="dropdown-item @if($listdata['count_used'][$view_data['id']] > 0) disabled @endif @if($level_user['deletejasa'] == 'No') disabled @endif" <?php if($listdata['count_used'][$view_data['id']] == 0){ if($level_user['deletejasa'] == 'Yes'){ ?> btn="del_data_{{$view_data['id']}}"<?php } }?>>Hapus Data</a>
																</div>
															</div>
														</td>
													</tr>
