            ...
            <?php if($res_user['level'] != 'LV5677001'){?>
                <tr>
                    ...
                    <td style="text-align: right; background-color: #FFFFFF; cursor: default; font-size:13px; font-weight: 600;" colspan="3" line="type_disc_po">
                        <select name="type_disc_po" style="width:50px;" <?php if($results['results']['qty_kirim'] > 0){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['kode_user'] != null && $results['results']['detail']['kode_user'] != $res_user['id'] && $res_user['level'] != 'LV5677001'){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['status_transaksi'] == 'Finish'){?>disabled="true"<?php } ?> <?php if($results['results']['counttransaksi'] != 0){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['status_transaksi'] == 'Finish'){?>disabled="true"<?php } ?>>
                            <option value="Persen">(%)</option>
                            <option value="Jumlah">(<?php echo $mata_uang;?>)</option>
                            <!-- <option value="Tidak Aktif">Tidak Aktif</option> -->
                        </select>

                        <input type="text" name="up_disc_po" value="<?php echo number_format($results['results']['detail']['diskon_persen'],2,",",".") ?>" onKeyPress="return goodchars(event,'0123456789,',this)" style="width: 50px; text-align:right; font-weight: 600;" <?php if($results['results']['qty_kirim'] > 0){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['kode_user'] != null && $results['results']['detail']['kode_user'] != $res_user['id'] && $res_user['level'] != 'LV5677001'){?>disabled="true"<?php } ?> <?php if($results['results']['counttransaksi'] != 0){?>readonly="false"<?php } ?> <?php if($results['results']['detail']['status_transaksi'] == 'Finish'){?>disabled="true"<?php } ?>/>
                    </td>
                </tr>
            <?php }else{ ?>
                <tr>
...
<script type="text/javascript">
    ...
    function savetotalpo(){
        ...
        $.ajax({
            type: "POST",
            url: "/cash/upsummarypenjualan?_token={{csrf_token()}}&token={{$request['token']}}&u={{$request['u']}}",
            ...
            success: function(data){
                if(data.status_message == 'success'){
                    window.location.href = "/cash/viewpenjualan?d={{$results['results']['detail']['nomor']}}&fc=summary";
                }else{
