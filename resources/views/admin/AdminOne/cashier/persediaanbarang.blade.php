			...
			@section('script')
				<script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        $('[line="btn_page_awal"]').attr('href','/cash/persediaanbarang?td={{$request['d']}}&page=1&vd={{ $count_vd }}&keysearch={{ $keysearch }}');

                        $('[line="btn_page_min"]').attr('href','/cash/persediaanbarang?td={{$request['d']}}&page={{ $results['current_page'] - 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}');

                        $('[line="btn_page_plus"]').attr('href','/cash/persediaanbarang?td={{$request['d']}}&page={{ $results['current_page'] + 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}');

						$('[line="btn_page_akhir"]').attr('href','/cash/persediaanbarang?td={{$request['d']}}&page={{ $results['last_page'] }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}');
                    });
					...
					function datefilter() {
						...
						if($('input[name="datefilterstart"]').val() != '' && $('input[name="datefilterend"]').val() != ''){
							window.location.href = "/cash/persediaanbarang?d={{$request['d']}}&page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+key_search;
						}else{
					...
					function exportdata(namedata) {
						window.location.href = "/cash/exportpersediaanbarang?page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+encodeURIComponent($('input[name="key-search"]').val());
					}
                </script>
            @endsection
@endsection