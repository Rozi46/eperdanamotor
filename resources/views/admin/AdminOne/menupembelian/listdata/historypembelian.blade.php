                ...
                <div class="container-fluid text-left">
                    <div class="row">
                        <div class="col-md-12 bg_page_main hd" line="hd_action">
                            <div class="col-md-12 hd_page_main">History Pembelian</div>
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['inputpembelianbarang'] == 'Yes')<a href="/admin/menupembelianbarang"><button type="button" class="btn btn-primary">Input Pembelian</button></a>@endif
										
										@if($level_user['exportpembelianbarang'] == 'Yes')<button type="button" class="btn btn-info back" onclick="exportdata()"><i class="fa fa-download"></i> Export Data</button>@endif
									</div>
								</div>
							</div>
                        </div>
										...
										<table class="table_view table-striped table-hover">
											<thead>
												<tr>
													...
													<th class="colleft" style="width:200px; text-align: center;">No. Pembelian</th>
													<th style="min-width:170px; text-align: center;">Pembelian Oleh</th>
													<th style="min-width:260px; text-align: center;">Nama Supplier</th>
													...
													<th style="min-width:150px; text-align: center;">Diskon Barang (-) <br> % - Rp</th>
													...
													<th style="min-width:125px; text-align: center;">Total Faktur</th>
													<th style="min-width:125px; text-align: center;">Diskon Faktur (-)</th>
													<th style="min-width:125px; text-align: center;">Cash Diskon (-)</th>
													...
													<th style="min-width:200px; text-align: center;">Perusahaan Pembeli</th>
                                                    ...
                                                    <script>
                                                        ...
                                                        <?php if($view_data['kode_user'] != null && $view_data['status_transaksi'] != 'Proses'){?>
                                                            function printdata_{{$no}}() {
                                                                window.open("/admin/printpurchaseorder?d={{$view_data['nomor']}}");
                                                            }
                                                        <?php } ?>
                                                    </script>
														...
														@foreach($produk_list as $index => $view_produk)
															<tr>
																@if($index === 0)
																	...
																	<br/><?php if($res_user['tipe_user'] == 'Super User'){?> <i class="fa fa-building" title="{{ $listdata['detail_perusahaan'][$view_data['code_data']]['kantor'] ?? 'Belum Ditentukan'}} | {{ $listdata['detail_cabang'][$view_data['code_data']]['nama_cabang'] ?? 'Belum Ditentukan'}}"></i><?php } ?>
																	</td>
																	<td rowspan="{{ $rowspan }}">{{ !empty($view_data['tanggal']) ? Date::parse($view_data['tanggal'])->format('d F Y') : 'Belum ditentukan' }}</td>
																	<td class="colleft link" rowspan="{{ $rowspan }}" style="text-align:center;" title="Detail" onclick="viewdata_{{$no}}()">{{ $view_data['nomor'] ?? 'Belum Ditentukan'}}</td>
																	<td rowspan="{{ $rowspan }}">{{ $view_data['nama_user_input'] ?? 'Belum Ditentukan' }}</td>
																	<td rowspan="{{ $rowspan }}">{{ $listdata['detail_supplier'][$view_data['code_data']]['nama'] ?? 'Belum ditentukan'}}</td>
																@endif

																<td>{{ $listdata['detail_produk'][$view_produk['kode_barang']]['nama'] ?? 'Belum ditentukan' }}</td>
																<td style="text-align:center;">{{ number_format($view_produk['jumlah_beli'] ?? 0,2,",",".") }}</td>
																...
																@if($index === 0)
																	...
																	<td style="text-align:right;" rowspan="{{ $rowspan }}">{{ number_format($view_data['diskonCash_harga'] ?? 0,2,",",".") }}</td>
																	<td style="text-align:right;" rowspan="{{ $rowspan }}">{{ number_format($view_data['grand_total'] ?? 0,2,",",".") }}</td>
																	<td rowspan="{{ $rowspan }}">{{ $view_data['ket'] }}</td>
																	...
																	<td rowspan="{{ $rowspan }}" style="text-align:center;">
																		@if($view_data['status_transaksi'] == 'Proses')
																	...
																	<td rowspan="{{ $rowspan }}">{{ $listdata['detail_cabang'][$view_data['code_data']]['nama_cabang'] ?? 'Belum Ditentukan'}}</td>
																	<!-- <td rowspan="{{ $rowspan }}" onclick="printdata_{{$no}}()" style="text-align:center; cursor: pointer;">
																		<i class="fa fa-print" title="Print"></i>
																	</td> -->
																	<td rowspan="{{ $rowspan }}" <?php if($view_data['status_transaksi'] != 'Proses'){?>class="colright link"<?php }else{ ?> class="colright" <?php } ?>style="text-align: center; <?php if($view_data['kode_user'] == null && $view_data['status_transaksi'] == 'Proses' ){?>cursor: not-allowed;<?php } ?> " onclick="printdata_{{$no}}()"><i class="colright <?php if($view_data['kode_user'] != null && $view_data['status_transaksi'] != 'Proses'){?>white<?php }else{ ?>head<?php } ?> fa fa-print" title="Print" style="text-align: center; <?php if($view_data['kode_user'] == null or $view_data['status_transaksi'] == 'Proses' ){?>cursor: not-allowed;<?php } ?> "></i></td>
																@endif
															</tr>
														@endforeach
													@else
														<tr>
															...
															<td style="text-align:left;">{{ $view_data['nama_user_input'] ?? 'Belum Ditentukan' }}</td>
															<td style="text-align:left;">{{ $listdata['detail_supplier'][$view_data['code_data']]['nama'] ?? 'Belum ditentukan'}}</td>
															<td colspan="7" style="text-align:center;">Tidak ada data yang tersedia</td>
															<!-- <td style="text-align:right;"><?php echo number_format($view_data['sub_total'] ?? 0,2,",",".") ?></td>
															<td style="text-align:right;"><?php echo number_format($view_data['ppn'] ?? 0,2,",",".") ?></td> -->
															<td style="text-align:right;"><?php echo number_format($view_data['total'] ?? 0,2,",",".") ?></td>
															<td style="text-align:right;"><?php echo number_format($view_data['diskon_harga'] ?? 0,2,",",".") ?></td>
															<td style="text-align:right;"><?php echo number_format($view_data['diskonCash_harga'] ?? 0,2,",",".") ?></td>
															<!-- <td style="text-align:right;"><?php echo number_format($view_data['biaya_kirim'] ?? 0,2,",",".") ?></td> -->
															<td style="text-align:right;"><?php echo number_format($view_data['grand_total'] ?? 0,2,",",".") ?></td>
															<td>{{$view_data['ket']}}</td>
															...
															<td>{{ $listdata['detail_cabang'][$view_data['code_data']]['nama_cabang'] ?? 'Belum Ditentukan'}}</td>
															<td <?php if($view_data['status_transaksi'] != 'Proses'){?>class="colright link"<?php }else{ ?> class="colright" <?php } ?>style="text-align: center; <?php if($view_data['kode_user'] == null && $view_data['status_transaksi'] == 'Proses' ){?>cursor: not-allowed;<?php } ?> " onclick="printdata_{{$no}}()"><i class="colright <?php if($view_data['kode_user'] != null && $view_data['status_transaksi'] != 'Proses'){?>white<?php }else{ ?>head<?php } ?> fa fa-print" title="Print" style="text-align: center; <?php if($view_data['kode_user'] == null or $view_data['status_transaksi'] == 'Proses' ){?>cursor: not-allowed;<?php } ?> "></i></td>
														</tr>
													@endif
												@empty
													<tr>
			...
			@section('script')
				<script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        $('[line="btn_page_awal"]').attr('href','/admin/historypembelianbarang?tp='+tipe_data+'&d={{$request['d']}}&page=1&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_min"]').attr('href','/admin/historypembelianbarang?tp='+tipe_data+'&d={{$request['d']}}&page={{ $results['current_page'] - 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_plus"]').attr('href','/admin/historypembelianbarang?tp='+tipe_data+'&d={{$request['d']}}&page={{ $results['current_page'] + 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

						$('[line="btn_page_akhir"]').attr('href','/admin/historypembelianbarang?tp='+tipe_data+'&d={{$request['d']}}&page={{ $results['last_page'] }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');
                    });
					...
					function datefilter() {
						...
						if($('input[name="datefilterstart"]').val() != '' && $('input[name="datefilterend"]').val() != ''){
							window.location.href = "/admin/historypembelianbarang?tp="+tipe_data+"&d={{$request['d']}}&page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+key_search+"&searchdate="+datefilterstart+"sd"+datefilterend;
						}else{
					...
					function exportdata() {
						...
						window.location.href = "/admin/exportpembelianbarang?tp="+tipe_data+"&page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+encodeURIComponent($('input[name="key-search"]').val())+searchdate;
					}
                </script>
            @endsection
@endsection