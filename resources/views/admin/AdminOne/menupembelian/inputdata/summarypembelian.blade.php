<?php if(count($results['results']['list_produk']) > 0){?>
    <?php if($results['results']['qty_pembelian'] == $results['results']['qty_terima']){?>
        <?php
            $mata_uang = 'Rp';
            $diskon = number_format($results['results']['detail']['diskon_persen'],2,",",".");
            $diskonCash = number_format($results['results']['detail']['diskonCash_persen'],2,",",".");
        ...
        <tr>
            ...
            <td class="strtable" style="text-align: right; background-color: #FFFFFF; cursor: default; font-size:13px; font-weight: 600;" colspan="3"><?php echo $diskonCash; ?></td>
        </tr>
        <tr>
    ...
    <?php }else{ ?> 
        <?php if($results['results']['detail']['status_transaksi'] != 'Finish' && $results['results']['qty_terima'] == 0){?>
            <?php
                $mata_uang = 'Rp';
            ?>
            <tr>
            ...
            <?php if($res_user['id'] != 'bd050931-d837-11eb-8038-204747ab6caa'){?>
                <tr>
                    ...
                    <td style="text-align: right; background-color: #FFFFFF; cursor: default; font-size:13px; font-weight: 600;" colspan="3" line="type_disc_po">
                        <select name="type_disc_po" style="width:50px;" <?php if($results['results']['qty_terima'] > 0){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['kode_user'] != null && $results['results']['detail']['kode_user'] != $res_user['id'] && $res_user['id'] != 'bd050931-d837-11eb-8038-204747ab6caa'){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['status_transaksi'] == 'Finish'){?>disabled="true"<?php } ?> <?php if($results['results']['counttransaksi'] != 0){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['status_transaksi'] == 'Finish'){?>disabled="true"<?php } ?>>
                            <option value="Persen">(%)</option>
                            <option value="Jumlah">(<?php echo $mata_uang;?>)</option>
                        </select>

                        <input type="text" name="up_disc_po" value="<?php echo number_format($results['results']['detail']['diskon_persen'],2,",",".") ?>" onKeyPress="return goodchars(event,'0123456789,',this)" style="width: 50px; text-align:right; font-weight: 600;" <?php if($results['results']['qty_terima'] > 0){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['kode_user'] != null && $results['results']['detail']['kode_user'] != $res_user['id'] && $res_user['id'] != 'bd050931-d837-11eb-8038-204747ab6caa'){?>disabled="true"<?php } ?> <?php if($results['results']['counttransaksi'] != 0){?>readonly="true"<?php } ?> <?php if($results['results']['detail']['status_transaksi'] == 'Finish'){?>disabled="true"<?php } ?>/>
                    </td>
                </tr>
                <tr>
                    <td style="text-align: right; background-color: #FFFFFF; cursor: default; font-size:13px; font-weight: 600;" colspan="6">Cash Diskon  :</td>
                    <td style="text-align: right; background-color: #FFFFFF; cursor: default; font-size:13px; font-weight: 600;" colspan="3" line="type_disc_cash">
                        <select name="type_disc_cash" style="width:50px;" <?php if($results['results']['qty_terima'] > 0){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['kode_user'] != null && $results['results']['detail']['kode_user'] != $res_user['id'] && $res_user['id'] != 'bd050931-d837-11eb-8038-204747ab6caa'){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['status_transaksi'] == 'Finish'){?>disabled="true"<?php } ?> <?php if($results['results']['counttransaksi'] != 0){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['status_transaksi'] == 'Finish'){?>disabled="true"<?php } ?>>
                            <option value="Persen">(%)</option>
                            <option value="Jumlah">(<?php echo $mata_uang;?>)</option>
                        </select>

                        <input type="text" name="up_disc_cash" value="<?php echo number_format($results['results']['detail']['diskon_persen'],2,",",".") ?>" onKeyPress="return goodchars(event,'0123456789,',this)" style="width: 50px; text-align:right; font-weight: 600;" <?php if($results['results']['qty_terima'] > 0){?>disabled="true"<?php } ?> <?php if($results['results']['detail']['kode_user'] != null && $results['results']['detail']['kode_user'] != $res_user['id'] && $res_user['id'] != 'bd050931-d837-11eb-8038-204747ab6caa'){?>disabled="true"<?php } ?> <?php if($results['results']['counttransaksi'] != 0){?>readonly="true"<?php } ?> <?php if($results['results']['detail']['status_transaksi'] == 'Finish'){?>disabled="true"<?php } ?>/>
                    </td>
                </tr>
            <?php }else{ ?>
                <tr>
                ...
                <tr>
                    <td style="text-align: right; background-color: #FFFFFF; cursor: default; font-size:13px; font-weight: 600;" colspan="6">Cash Diskon  :</td>
                    <td style="text-align: right; background-color: #FFFFFF; cursor: default; font-size:13px; font-weight: 600;" colspan="3" line="type_disc_cash">
                        <select name="type_disc_cash" style="width:50px;" <?php if($results['results']['counttransaksi'] != 0){?>disabled="true"<?php } ?>>
                            <option value="Persen">(%)</option>
                            <option value="Jumlah">(<?php echo $mata_uang;?>)</option>
                        </select>

                        <input type="text" name="up_disc_cash" value="<?php echo number_format($results['results']['detail']['diskonCash_persen'],2,",",".") ?>" onKeyPress="return goodchars(event,'0123456789,',this)" style="width: 50px; text-align:right; font-weight: 600;" <?php if($results['results']['counttransaksi'] != 0){?>readonly="true"<?php } ?>/>
                    </td>
                </tr>
            <?php } ?>
            <tr>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('select[name="type_disc_po"]').change(function(){
            ...
            if(type_disc_po == 'Tidak Aktif'){
                ...
                if(type_disc_po == 'Jumlah'){
                    $('[line="type_disc_po"]').css({'width':'165px'});
                    $('input[name="up_disc_po"]').css({'width':'105px'});
                    $('input[name="up_disc_po"]').val('0,00').prop("readonly", false); $('input[name="up_disc_po"]').focus().select().val('<?php echo number_format($results['results']['detail']['diskon_harga'],2,",",".") ?>');
                }else{
                    $('[line="type_disc_po"]').css({'width':'auto'});
                    $('input[name="up_disc_po"]').css({'width':'50px'});
                    $('input[name="up_disc_po"]').val('0,00').prop("readonly", false); $('input[name="up_disc_po"]').focus().select().val('<?php echo number_format($results['results']['detail']['diskon_persen'],2,",",".") ?>');
                }
            }
        });
        ...
        var type_disc_cash = $('select[name="type_disc_cash"]').val();
        if(type_disc_cash == 'Jumlah'){
            $('[line="type_disc_cash"]').css({'width':'165px'});
            $('input[name="up_disc_cash"]').css({'width':'105px'});
        }else{
            $('[line="type_disc_cash"]').css({'width':'auto'});
            $('input[name="up_disc_cash"]').css({'width':'50px'});
        }

        $('select[name="type_disc_cash"]').change(function(){
            var type_disc_cash = $('select[name="type_disc_cash"]').val();
            if(type_disc_cash == 'Tidak Aktif'){
                $('[line="type_disc_cash"]').css({'width':'auto'});
                $('input[name="up_disc_cash"]').css({'width':'50px'});
                $('input[name="up_disc_cash"]').val('0,00').prop("readonly", true); 
                savetotalcash();  
            }else{
                if(type_disc_cash == 'Jumlah'){
                    $('[line="type_disc_cash"]').css({'width':'165px'});
                    $('input[name="up_disc_cash"]').css({'width':'105px'});
                    $('input[name="up_disc_cash"]').val('0,00').prop("readonly", false); $('input[name="up_disc_cash"]').focus().select().val('<?php echo number_format($results['results']['detail']['diskonCash_harga'],2,",",".") ?>');
                }else{
                    $('[line="type_disc_cash"]').css({'width':'auto'});
                    $('input[name="up_disc_cash"]').css({'width':'50px'});
                    $('input[name="up_disc_cash"]').val('0,00').prop("readonly", false); $('input[name="up_disc_cash"]').focus().select().val('<?php echo number_format($results['results']['detail']['diskonCash_persen'],2,",",".") ?>');
                }
            }
        });

        $('input[name="up_disc_cash"]').change(function(){
            savetotalcash();
        });
    });
    ...
    function savetotalpo(){
        ...
        $.ajax({
            type: "POST",
            url: "/admin/upsummarypembelian?_token={{csrf_token()}}&token={{$request['token']}}&u={{$request['u']}}",
            ...
            success: function(data){
                if(data.status_message == 'success'){
                    window.location.href = "/admin/viewpembelian?d={{$results['results']['detail']['nomor']}}&fc=summary";
                }else{
    ...
    function savetotalcash(){
        var type_disc_cash = $('select[name="type_disc_cash"]').val();
        var up_disc_cash = $('input[name="up_disc_cash"]').val();

        if(type_disc_cash == 'Tidak Aktif'){
            var up_disc_cash = '0,00';
            $('input[name="up_disc_cash"]').val(up_disc_cash);
        }

        var up_disc_cash = up_disc_cash.replace(".", "");
        ...
        $.ajax({
            type: "POST",
            url: "/admin/upsummarypembeliancash?_token={{csrf_token()}}&token={{$request['token']}}&u={{$request['u']}}",
            data:"code_data=<?php echo $results['results']['detail']['code_data'];?>&nilai_diskon="+up_disc_cash+"&tipe_diskon="+type_disc_cash,
            cache: false,
            success: function(data){
                if(data.status_message == 'success'){
                    window.location.href = "/admin/viewpembelian?d={{$results['results']['detail']['nomor']}}&fc=summary";
