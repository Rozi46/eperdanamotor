                ...
                <div class="container-fluid text-left">
                    <div class="row">
                        <div class="col-md-12 bg_page_main hd">
                            <div class="col-md-12 hd_page_main">History Pengiriman</div>
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['inputpengirimanbarang'] == 'Yes')<a href="/admin/menupengirimanbarang"><button type="button" class="btn btn-primary">Input Pengiriman</button></a>@endif
										
										@if($level_user['exportpengirimanbarang'] == 'Yes')<button type="button" class="btn btn-info back" onclick="exportdata('pengirimanbarang')"><i class="fa fa-download"></i> Export Data</button>@endif
									</div>
								</div>
							</div>
                        </div>
										...
										<table class="table_view table-striped table-hover">
											<thead>
												<tr>
													...
													<th class="colleft" style="width:200px; text-align: center;">No. Pengiriman</th>
                                                    <th style="min-width:175px; text-align: center;">Nama Customer</th>
                                                    <th style="min-width:175px; text-align: center;">Penjualan Oleh</th>
													<th style="width:125px; text-align: center;">No. Penjualan</th>
												...
												<?php $no = 0;?> @forelse($results['data'] as $view_data) <?php $no++ ;?>
													<script>
														function viewdata_{{$no}}() {
															loadingpage(2000);
															window.location.href = "/admin/viewpengiriman?d={{$view_data['nomor_pengiriman']}}";
														}
                                                        <?php if($view_data['status_transaksi'] != 'Proses'){?>
                                                            function printdata_{{$no}}() {
                                                                window.open("/admin/printpengiriman?d={{$view_data['nomor_pengiriman']}}");
                                                            }
                                                        <?php } ?>
													</script>
													<tr>
														...
														<td class="colleft link" style="text-align:center;" title="Detail"  onclick="viewdata_{{$no}}()">{{ $view_data['nomor_pengiriman'] ?? 'Belum Ditentukan' }}</td>
                                                        <td>{{ $listdata['detail_customer'][$view_data['nomor_pengiriman']]['nama'] ?? 'Belum Ditentukan'}}</td>
                                                        <td>{{ $listdata['user_penjualan'][$view_data['nomor_pengiriman']]['full_name'] ?? 'Belum Ditentukan'}}</td>
                                                        <td>{{ $listdata['detail_penjualan'][$view_data['nomor_pengiriman']]['nomor'] ?? 'Belum Ditentukan'}}</td>
														<td style="text-align:left;">{{ $view_data['ket'] ?? 'Belum Ditentukan'}}</td>
														<td style="text-align:center;">{{ number_format($listdata['qty_pengiriman'][$view_data['nomor_pengiriman']] ?? 0,0,"",".") }}</td>
                                                        <td>{{ $listdata['detail_gudang'][$view_data['nomor_pengiriman']]['nama'] ?? 'Belum Ditentukan' }}</td>
			...
			@section('script')
				<script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        $('[line="btn_page_awal"]').attr('href','/admin/historypengiriman?tp='+tipe_data+'&d={{$request['d']}}&page=1&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_min"]').attr('href','/admin/historypengiriman?tp='+tipe_data+'&d={{$request['d']}}&page={{ $results['current_page'] - 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_plus"]').attr('href','/admin/historypengiriman?tp='+tipe_data+'&d={{$request['d']}}&page={{ $results['current_page'] + 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

						$('[line="btn_page_akhir"]').attr('href','/admin/historypengiriman?tp='+tipe_data+'&d={{$request['d']}}&page={{ $results['last_page'] }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');
                    });
					...
					function datefilter() {
						...
						if($('input[name="datefilterstart"]').val() != '' && $('input[name="datefilterend"]').val() != ''){
							window.location.href = "/admin/historypengiriman?tp="+tipe_data+"&d={{$request['d']}}&page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+key_search+"&searchdate="+datefilterstart+"sd"+datefilterend;
						}else{
					...
					function exportdata() {
						...
						window.location.href = "/admin/exportpengirimanbarang?tp="+tipe_data+"&page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+encodeURIComponent($('input[name="key-search"]').val())+searchdate;
					}
                </script>
            @endsection
@endsection