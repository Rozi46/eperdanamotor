...
@section('title', 'History Mutasi Kirim')
                ...
                <div class="container-fluid text-left">
                    <div class="row">
                        <div class="col-md-12 bg_page_main hd">
                            <div class="col-md-12 hd_page_main">History Mutasi Kirim</div>
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['inputmutasikirim'] == 'Yes')<a href="/admin/mutasikirim"><button type="button" class="btn btn-primary">Input Mutasi Kirim</button></a>@endif
										
										@if($level_user['exportmutasikirim'] == 'Yes')<button type="button" class="btn btn-info back" onclick="exportdata()"><i class="fa fa-download"></i> Export Data</button>@endif
									</div>
								</div>
							</div>
                        </div>
										...
										<table class="table_view table-striped table-hover">
											<thead>
												<tr>
													...
													<th class="colleft" style="width:200px; text-align: center;">No. Mutasi</th>
												...
												<?php $no = 0;?> @forelse($results['data'] as $view_data) <?php $no++ ;?>				
                                                    <script>
                                                        function viewdata_{{$no}}() {
                                                            loadingpage(2000);
                                                            window.location.href = "/admin/viewmutasikirim?d={{$view_data['nomor']}}";
                                                        }
                                                        <?php if($view_data['kode_user'] != null && $view_data['status_transaksi'] != 'Input'){?>
                                                            function printdata_{{$no}}() {
                                                                window.open("/admin/printmutasikirim?d={{$view_data['nomor']}}");
                                                            }
                                                        <?php } ?>
                                                    </script>
													<tr>
														...
														<td style="text-align:center;">{{ number_format($listdata['qty_kirim'][$view_data['code_data']] ?? 0,2,",",".") }}</td>
													    ...
													    <td <?php if($view_data['status_transaksi'] != 'Input'){?>class="colright link"<?php }else{ ?> class="colright" <?php } ?>style="text-align: center; <?php if($view_data['kode_user'] == null && $view_data['status_transaksi'] == 'Input' ){?>cursor: not-allowed;<?php } ?> " onclick="printdata_{{$no}}()"><i class="colright <?php if($view_data['kode_user'] != null && $view_data['status_transaksi'] != 'Input'){?>white<?php }else{ ?>head<?php } ?> fa fa-print" title="Print" style="text-align: center; <?php if($view_data['kode_user'] == null or $view_data['status_transaksi'] == 'Input' ){?>cursor: not-allowed;<?php } ?> "></i></td>
													</tr>
												@empty
													<tr>
			...
			@section('script')
				<script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        $('[line="btn_page_awal"]').attr('href','/admin/historymutasikirim?tp='+tipe_data+'&d={{$request['d']}}&page=1&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_min"]').attr('href','/admin/historymutasikirim?tp='+tipe_data+'&d={{$request['d']}}&page={{ $results['current_page'] - 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

                        $('[line="btn_page_plus"]').attr('href','/admin/historymutasikirim?tp='+tipe_data+'&d={{$request['d']}}&page={{ $results['current_page'] + 1 }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');

						$('[line="btn_page_akhir"]').attr('href','/admin/historymutasikirim?tp='+tipe_data+'&d={{$request['d']}}&page={{ $results['last_page'] }}&vd={{ $count_vd }}&keysearch={{ $keysearch }}&{{ $searchdate ?? '' }}');
                    });
					...
					function datefilter() {
						...
						if($('input[name="datefilterstart"]').val() != '' && $('input[name="datefilterend"]').val() != ''){
							window.location.href = "/admin/historymutasikirim?tp="+tipe_data+"&d={{$request['d']}}&page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+key_search+"&searchdate="+datefilterstart+"sd"+datefilterend;
						}else{
					...
					function exportdata() {
						...
						window.location.href = "/admin/exportmutasikirim?tp="+tipe_data+"&page=1&vd="+encodeURIComponent($('input[id="countvd"]').val())+"&keysearch="+encodeURIComponent($('input[name="key-search"]').val())+searchdate;
					}
                </script>
            @endsection
@endsection