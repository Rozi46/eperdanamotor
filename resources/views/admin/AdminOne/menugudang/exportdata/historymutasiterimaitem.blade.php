                ...
                <?php $no = 0;?> @forelse($results['data'] as $view_data) <?php $no++ ;?>
                    <tr>
                        ...
                        <td class="strtable" style="text-align:center;">
                            {{ $listdata['detail_barang'][$view_data['id']]['nama'] != 'null' ? $listdata['detail_barang'][$view_data['id']]['nama'] : 'Belum ditentukan' }}
                        </td>                       
                        <td class="strtable" style="text-align:center;">{{ $listdata['detail_satuan'][$view_data['id']]['nama'] ?? 'Belum ditentukan'}}</td>
                        <td class="strtable" style="text-align:center;">{{ $view_data['jumlah_terima'] ?? 0 }}</td>
                        <td class="strtable" style="text-align:center;">
                            {{ $listdata['detail_gudang_asal'][$view_data['id']]['nama'] != 'null' ? $listdata['detail_gudang_asal'][$view_data['id']]['nama'] : 'Belum ditentukan' }}
                        </td>
                        <td class="strtable" style="text-align:center;">
                            {{ $listdata['detail_gudang_tujuan'][$view_data['id']]['nama'] != 'null' ? $listdata['detail_gudang_tujuan'][$view_data['id']]['nama'] : 'Belum ditentukan' }}
                        </td>
