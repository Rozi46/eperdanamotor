								...
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
                                        ...
                                        @if($level_user['inputpenerimaanbarang'] == 'Yes')
                                            <?php if($results['results']['detail_pembelian']['status_transaksi'] == 'Proses'){?>
                                        ...
                                        @if($level_user['historypenerimaanbarang'] == 'Yes')
                                            <?php if($results['results']['detail_pembelian']['status_transaksi'] == 'Finish'){?>
                                                <a href="printpenerimaan?d={{$results['results']['detail']['nomor_penerimaan']}}" target="_blank"><button type="button" class="btn btn-secondary" name="btn_print" btn="print_data"><i class="fa fa-print"></i> Print Tanda Terima</button></a>
                                                <?php if($res_user['id'] == 'bd050931-d837-11eb-8038-204747ab6caa'){?> <button type="button" class="btn btn-danger btn_del" name="del_data" btn="del_data"><i class="fa fa-trash-o"></i> Hapus Penerimaan</button> <?php } ?>
                                            <?php }?>
                                        @endif    
									</div>
								</div>
								...
								<form method="post" name="form_data" enctype="multipart/form-data" action="/admin/savepenerimaan">
									...
									<div class="row bg_data_page form_page content">
										...
										<div class="col-md-4 bg_form_page">
                                            <div class="form-group row form_input text-left">
                                                <label for="tgl_transaksi" class="col-sm-6 col-form-label">Tanggal Penerimaan</label>
										...
										<div class="col-md-4 bg_form_page">
                                            <div class="form-group row form_input text-left">
                                                <label for="code_transaksi" class="col-sm-4 col-form-label">No. Penerimaan</label>
                                                <div class="col-sm-8 input">
                                                    <input type="text" name="code_transaksi" placeholder="No. Penerimaan" value="">
                                                </div>
                                            </div>
                                        </div>
										<div class="col-md-4 bg_form_page">
										...
										<div class="col-md-4 bg_form_page">
                                            <div class="form-group row form_input text-left">
                                                <label for="no_pembelian" class="col-sm-6 col-form-label">No. Pembelian (PO)</label>
                                                <div class="col-sm-6 input">
                                                    <input type="text" name="no_pembelian" placeholder="No. Pembelian (PO)" value="">
                                                </div>
                                            </div>
                                        </div>
			...
			@section('script')
				<script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        $('input[name="code_transaksi"]').val('{{ $results['results']['detail']['nomor_penerimaan'] ?? 'Belum Ditentukan' }}');
                        $('input[name="in_code_transaksi"]').val('{{ $results['results']['detail']['nomor_penerimaan'] ?? '' }}');
                        
                        $('input[name="no_pembelian"]').val('{{ $results['results']['detail_pembelian']['nomor'] ?? 'Belum Ditentukan' }}');
                        $('input[name="in_no_pembelian"]').val('{{ $results['results']['detail_pembelian']['nomor'] ?? '' }}');
                        ...
                        $.get("/admin/listprodpenerimaan",{code_data:'{{$results['results']['detail']['code_data']}}',status_data:'Yes',nomor_pembelian:'{{$results['results']['detail']['nomor_pembelian']}}',nomor_penerimaan:'{{$results['results']['detail']['nomor_penerimaan']}}'},function(listproduk){
                        ...
                        $('[name="btn_cancel"]').click(function(){
                            if($('[name="btn_cancel"]').click){
                                var no_penerimaan = $('input[name="in_code_transaksi"]').val();
                                if(no_penerimaan != ''){
                                    $('div[data-model="confirmasi"]').modal({backdrop: false});
                                    $('div[data-model="confirmasi"] .modal-body').html('<div class="alert alert-danger">Anda yakin untuk membatalkan penerimaan barang {{$results['results']['detail']['nomor_penerimaan']}}.</div>');
                                    ...
                                    $('button[btn-action="aciton-confirmasi"]').click(function(){
                                        if($('button[btn-action="aciton-confirmasi"]').click){
                                            ...
                                            loadingpage(20000);
                                            window.location.href = "/admin/deleterdobarang?d={{$code_data}}&tipe_data=input&cdpo="+no_penerimaan;
                                        }
                                    });
                                }
                            }
                        ...
                        $('[name="del_data"]').click(function(){
                            if($('[name="del_data"]').click){
                                ...
                                if(no_penerimaan != ''){
                                    $('div[data-model="confirmasi"]').modal({backdrop: false});
                                    $('div[data-model="confirmasi"] .modal-body').html('<div class="alert alert-danger">Anda yakin untuk menghapus penerimaan barang {{$results['results']['detail']['nomor_penerimaan']}}.</div>');
                                    $('button[btn-action="aciton-confirmasi"]').remove();
                                    $('button[btn-action="close-confirmasi"]').before('<button type="button" class="btn btn-primary btn-sm" btn-action="aciton-confirmasi">Yakin</button>');
                                    $('button[btn-action="aciton-confirmasi"]').click(function(){
                                        if($('button[btn-action="aciton-confirmasi"]').click){
                        ...
                        $('[btn="save_data"]').click(function(){
                            if($('[btn="save_data"]').click){
                                ...
                                <?php if(count($results['results']['list_produk']) > 0){?>
                                    $('div[data-model="confirmasi"] .modal-body').html('<div class="alert alert-warning">Anda yakin untuk simpan dan selesaikan penerimaan barang {{$results['results']['detail']['nomor_penerimaan']}}. Setelah simpan dan selesai maka data tidak bisa diubah kembali.</div>');
                                    $('button[btn-action="aciton-confirmasi"]').remove();
                                    $('button[btn-action="close-confirmasi"]').before('<button type="button" class="btn btn-primary btn-sm" btn-action="aciton-confirmasi">Yakin</button>');
                                    $('button[btn-action="aciton-confirmasi"]').click(function(){
                                        if($('button[btn-action="aciton-confirmasi"]').click){
                                            ...
                                            window.location.href = "/admin/updaterdobarang?code_data={{$results['results']['detail']['code_data']}}&tipe_data=input&keterangan="+encodeURIComponent(keterangan);                               
                                            
                                            // $.ajax({
                                            //     type: "POST",
                                            //     url: "/admin/updaterdobarang?_token={{csrf_token()}}&token={{$request['token']}}&u={{$request['admin_login']}}",
                                            //     data:"code_data={{$results['results']['detail']['code_data']}}&tipe_data=input&keterangan="+encodeURIComponent(keterangan),
                                            //     cache: false,
                                            //     success: function(data){
                                            //         if(data.status_message == 'success'){    
                                            //                 window.location.href = "/admin/menupenerimaanbarang";
                                            //         }else{
